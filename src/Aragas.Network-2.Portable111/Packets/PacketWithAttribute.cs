...
namespace Aragas.Network.Packets
{
    ...
    public abstract class PacketAttribute<TNumberType, TSerializer, TDeserializer> : Packet<TNumberType, TSerializer, TDeserializer> where TNumberType : struct where TSerializer : PacketSerializer where TDeserializer : PacketDeserializer
    {
        private TNumberType? _id;
        public sealed override TNumberType ID
        {
            get
            {
                if(_id == null)
                    // casting to dynamic will allow us to use the explicit casting overrides
                    _id = (TNumberType) (dynamic) GetType().GetTypeInfo().GetCustomAttribute<PacketAttribute>().ID;
                return _id.Value;
            }
        }
    }
